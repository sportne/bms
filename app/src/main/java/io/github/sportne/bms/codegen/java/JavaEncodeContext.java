...
/**
 * Shared state used while emitting Java encode statements.
 *
...
record JavaEncodeContext(
    ...
    String ownerPrefix) {
  /**
   * Creates one Java encode emission context.
   *
   * @param builder destination source builder
   * @param messageType message currently being generated
   * @param generationContext reusable resolved-type lookup maps
   ...
   */
  JavaEncodeContext withOwnerPrefix(String newOwnerPrefix) {
    return new JavaEncodeContext(
